<template>
    ...
    >
        <a-form :form="form">
            ...
            <a-divider></a-divider>
            <h6 v-if="orderMatchWebCouponList.length>0 && orderMatchCouponList.length>0">温馨提示：当两种优惠策略同时存在时按先酒店优惠后网站优惠计算，请按照先酒店后网站的顺序勾选</h6>
            <h2 v-if="orderMatchCouponList.length>0">酒店优惠</h2>
            ...
            <h2 v-if="orderMatchWebCouponList.length>0">网站优惠</h2>
            <a-checkbox-group v-model="checkedList1" @change="onchange1">
                <a-table
                        :columns="columns"
                        :dataSource="orderMatchWebCouponList"
                        :showHeader="false"
                        bordered
                        v-if="orderMatchWebCouponList.length>0"
                >
                    <a-checkbox
                            slot="id"
                            slot-scope="record"
    ...
    export default {
        ...
        methods: {
            ...
            changeRoomNum(v) {
                ...
                this.finalPrice = this.totalPrice
                this.afterHotelPrice = this.totalPrice
            },
            onchange() {
                this.currentPrice = this.totalPrice
                if (this.checkedList.length > 0) {
                    this.orderMatchCouponList.filter(item => this.checkedList.indexOf(item.id) != -1, item => this.checkedList.indexOf(item.discountMoney != 0)).forEach(item => this.currentPrice = this.currentPrice - item.discountMoney)
                    this.orderMatchCouponList.filter(item => this.checkedList.indexOf(item.id) != -1, item => this.checkedList.indexOf(item.discount != 0)).forEach(item => this.currentPrice = this.currentPrice * (1.0 - item.discount))
                }
                this.finalPrice = this.currentPrice
                this.afterHotelPrice = this.currentPrice
            },
            onchange1() {
                this.currentPrice = this.afterHotelPrice
                if (this.checkedList1.length > 0) {
                    this.orderMatchWebCouponList.filter(item => this.checkedList1.indexOf(item.id) != -1, item => this.checkedList1.indexOf(item.discount != 0)).forEach(item => this.currentPrice = this.currentPrice * (1.0 - item.discount))
                }
                this.finalPrice = this.currentPrice
            },
            handleSubmit(e) {
                ...
                this.form.validateFieldsAndScroll((err, values) => {
                    if (!err) {
                        const data = {
                            ...
                            createDate: moment(this.curTime).format('YYYY-MM-DD'),
                            // price: this.checkedList.length > 0 && this.checkedList1.length > 0 ? this.finalPrice : this.totalPrice
                            price: (!(this.checkedList.length > 0) || !(this.checkedList1.length > 0)) ? this.totalPrice : this.finalPrice
                        }
                        this.addOrder(data)
                    }
                });
        ...
        watch: {
            totalPrice(val) {
                ...
                this.getOrderMatchCoupons(dataH)
                let dataW = {
                    userId: this.userId,
                    hotelId: -1,
                    orderPrice: this.totalPrice,
                    ...
                    checkOut: moment(this.form.getFieldValue('date')[1]).format('YYYY-MM-DD'),
                    createDate: moment(this.curTime).format('YYYY-MM-DD'),
                    bizRegKnownByHotelId: this.currentHotelId,
                }
                this.getOrderMatchWebCoupons(dataW)
            }
        }
    }
</script>